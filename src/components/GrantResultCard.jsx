    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 12, minHeight: 70 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: 14, fontWeight: 550, color: '#4d554a', marginTop: 5, lineHeight: 1.4, letterSpacing: 0, display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: 2, overflow: 'hidden' }}>{grant.agency}</p>
        </div>
        <button
          type="button"
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, minHeight: 42, marginBottom: 10 }}>
        ...
        <p style={{ fontSize: 15, fontWeight: 650, color: '#076818', lineHeight: 1.4, letterSpacing: 0, display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: 2, overflow: 'hidden' }}>{grant.subtitle}</p>
      </div>
      ...
      {reasons.length > 0 && (
        <div className="no-scrollbar" style={{ background: '#FFFFFF', borderRadius: 14, padding: '10px 14px', marginBottom: 10, minHeight: 72, overflowY: 'auto', boxSizing: 'border-box' }}>
          <p style={{ fontSize: 14, fontWeight: 650, color: '#4d554a', lineHeight: 1.4, marginBottom: 7 }}>해당 이유</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {reasons.map((reason) => (
              <div key={reason} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <p style={{ fontSize: 14, fontWeight: 550, color: '#4d554a', lineHeight: 1.4 }}>{reason}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ marginTop: 'auto', padding: '8px 0 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <CalendarDays size={16} color="#076818" strokeWidth={2.2} />
            <p style={{ fontSize: 14, fontWeight: 650, color: '#1f2433', lineHeight: 1.4 }}>신청 기간</p>
          </div>
          <p style={{ fontSize: 14, fontWeight: 550, color: '#4d554a', lineHeight: 1.4 }}>{grant.period}</p>
        </div>
        <div style={{
          display: 'flex',
          alignItems: 'center',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
            <Clock size={15} color={countdownColor} strokeWidth={2} />
            <span>마감까지</span>
          </div>
